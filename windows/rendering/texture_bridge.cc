...
void TextureBridge::StopInternal() {
  if (is_running_) {
    ...
    auto closable =
        capture_session_.Get();
    if (closable) {
      Microsoft::WRL::ComPtr<ABI::Windows::Foundation::IClosable> closable_iface;
      if (SUCCEEDED(capture_session_.As(&closable_iface)) && closable_iface) {
        closable_iface->Close();
      }
    }
...
void TextureBridge::OnFrameArrived() {
  ...
  Microsoft::WRL::ComPtr<ABI::Windows::Graphics::Capture::IDirect3D11CaptureFrame>
      frame;
  auto hr = frame_pool_->TryGetNextFrame(frame.GetAddressOf());
  if (SUCCEEDED(hr) && frame) {
    Microsoft::WRL::ComPtr<
        ABI::Windows::Graphics::DirectX::Direct3D11::IDirect3DSurface>
        frame_surface;

    if (SUCCEEDED(frame->get_Surface(frame_surface.GetAddressOf()))) {
      Microsoft::WRL::ComPtr<IInspectable> inspectable;
      if (SUCCEEDED(frame_surface.As(&inspectable))) {
        last_frame_ =
            util::TryGetDXGIInterfaceFromObject<ID3D11Texture2D>(inspectable);
        has_frame = last_frame_ && !ShouldDropFrame();
      }
    }
  }
